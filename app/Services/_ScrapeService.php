...
class ScrapeService
{
  ...
  public static function scrapeProductsTemp($house)
  {
    print_r('Starting scraper...');        
    
      //get auction finished status & title
      $auction_date  ='Ended on July 4, 2011';
      $auction_title ='The Third Auction';
      //check if finished && new auction
      
        // auction data
        $auction_date =substr( $auction_date, 9);
        $auction_lots ='78' ;
        $auction_url ='auctions/18_the-third-auction/';

        // check IsExistAuctionAndHouseId
        ...
        if (!$check_auction_house) {          
          ...
          $auction->end_date =date('Y-m-d',strtotime($auction_date));
        ...
        // var_dump($auction_date, $auction_title, $auction_url, $auction_lots);
        ...
        //*******************************
        for ($i=1; $i < count($subPages); $i++) // (1~190)
        {
          ...
          { 
            ...
            if ($product_lot_num=='003073' || $product_lot_num=='003053')
            {
              // get product title
              ...
              // var_dump($product_title, $product_title_data, $product_price, $product_detail_url, $master_img);
              ...
              $product_description =$productCrawler->filter('.productinfo .productdescription p')->each(function ($node) 
              {  
                return $result[] = $node->text();
              });
              if (empty($product_description)) {
                $product_description_data['description']
                   = $productCrawler->filter('.productinfo .productdescription')->text();
              }else{
                // db(age ~ bottle_num) from $proDes
                $product_description_data = StringService::get_from_description($product_description);
              }
              ...
              if (isset($product_title_data['distillery_brand'])) 
                  $product['distillery_brand'] = $product_title_data['distillery_brand'];
              if (isset($product_title_data['bottler'])) 
                  $product['bottler'] = $product_title_data['bottler'];
              if (isset($product_title_data['series'])) 
                  $product['series'] = $product_title_data['series'];

              if (isset($product_description_data['age']))
                  $product['age'] = $product_description_data['age'];
              if (isset($product_description_data['bottled_for'])) 
                  $product['bottled_for'] = $product_description_data['bottled_for'];

              if (isset($product_description_data['abv'])) 
                  $product['abv'] = $product_description_data['abv'];
                
              if (isset($product_description_data['size'])) 
                  $product['size'] = $product_description_data['size'];
              if (isset($product_description_data['distilled'])) 
                  $product['distilled'] = $product_description_data['distilled'];
              if (isset($product_description_data['bottled'])) 
                  $product['bottled'] = $product_description_data['bottled'];
              ...
              if (isset($product_description_data['bottle_num'])) 
                  $product['bottle_num'] = $product_description_data['bottle_num'];
              if (isset($product_description_data['num_bottles'])) 
                  $product['num_bottles'] = $product_description_data['num_bottles'];
              $product['description'] = $product_description_data['description'];
    ...
    $crawler->each(function ($node) use ($house) 
    {
        ...
        /*&& !Auction::IsExist($auction_title) && !Auction::IsExistHouseId($house['id'])*/) 
      {
        ...
        //Check if scraped auction?
          ...
          $lastIndex = count($subPages) - 1;
          if ($lastIndex == -1) {
            $surfix_url = $auction_url;
            $lastIndex = 0;
          }else{
            $surfix_url = $subPages[$lastIndex];
          }
    ...
    $crawler->each(function ($node) use ($house) 
    {
      ...
      if (OtherFunc::checkIfContainStr($auction_title,'Auction'))
      {
        ...
        //Check if scraped auction?
          ...
          $current =Product::CountProductOfAuction($auction_id);
          //get count transactions from real
          $lastIndex = (int) $last_page_num;
          if ($lastIndex == 0) {
            $surfix_url = $subpage_prefix;
          }else{
            $surfix_url = $subpage_prefix.'&page='.$lastIndex;
          }
          $lastpageCrawler = Goutte::request('GET',$house['site'].$surfix_url)
        ...
        for ($i=0; $i<=(int) $last_page_num; $i++) // (1~last)
        {
          ...
          $subpageCrawler = Goutte::request('GET',$house['site'].$sub_page_url);
          $subpageCrawler->filter('.block-views')->eq(1)->filter('.views-row')
                          ->each(function ($node) use ($auction_id, $house) 
          { 
